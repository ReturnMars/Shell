<template>
  <div class="bg-white border-b border-gray-200 flex items-center">
    <!-- 标签页列表 -->
    <div class="flex-1 flex overflow-x-auto h-[45px]">
      <div
        v-for="tab in tabs"
        :key="tab.id"
      ...
      >
        ...
        <!-- 标签页标题 -->
        <span
          class="text-sm font-medium mr-1.5 whitespace-nowrap overflow-hidden text-ellipsis"
        >
          {{ tab.title }}
        </span>
        ...
        <!-- 关闭按钮 -->
        <n-button quaternary circle size="tiny" @click.stop="closeTab(tab)">
          <template #icon>
