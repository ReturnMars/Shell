...
use crate::utils::{crypto, storage};
use serde_json;
...
impl ConnectionStorage {
        ...
        if let Some(mut config) = connections.get(connection_id).cloned() {
            ...
            if let Some(encrypted_password) = &config.password {
                let decrypted_password =
                    crypto::decrypt_password(encrypted_password, &self.master_key)?;
    ...
    fn load_connections(&self) -> Result<HashMap<String, ConnectionConfig>, String> {
        ...
        let connections: HashMap<String, ConnectionConfig> =
            serde_json::from_str(&content).map_err(|e| format!("解析连接配置文件失败: {}", e))?;
    ...
    fn load_or_generate_master_key(config_dir: &PathBuf) -> Result<Vec<u8>, String> {
        ...
        if key_file.exists() {
            // 加载现有密钥
            let key_data = fs::read(&key_file).map_err(|e| format!("读取主密钥失败: {}", e))?;
            Ok(key_data)
        } else {
            // 生成新密钥
            let master_key = crypto::generate_master_key();
            fs::write(&key_file, &master_key).map_err(|e| format!("保存主密钥失败: {}", e))?;
    ...
    pub fn export_connections(&self) -> Result<String, String> {
        ...
        serde_json::to_string_pretty(&export_data).map_err(|e| format!("导出连接配置失败: {}", e))
    }

    /// 导入连接配置（自动处理名称重复，生成唯一名称）
    pub fn import_connections(&self, json_data: &str) -> Result<(), String> {
        let imported: HashMap<String, ConnectionConfig> =
    ...
    pub fn save_tabs(&self, tabs: &HashMap<String, TabInfo>) -> Result<(), String> {
        ...
        fs::write(&self.tabs_file, json_data).map_err(|e| format!("保存标签页文件失败: {}", e))?;
    ...
    pub fn load_tabs(&self) -> Result<HashMap<String, TabInfo>, String> {
        ...
        let tabs: HashMap<String, TabInfo> =
            serde_json::from_str(&json_data).map_err(|e| format!("解析标签页数据失败: {}", e))?;
