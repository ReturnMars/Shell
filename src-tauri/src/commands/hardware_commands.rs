...
pub async fn get_hardware_info(connection_id: String) -> Result<HardwareInfo, String> {
    ...
    log::info!("当前所有连接数量: {}", connections.len());
    for conn in &connections {
        log::info!("连接配置 - ID: {}, 名称: {}, 主机: {}", 
                   conn.id, conn.name, conn.host);
    }
    
    // 调试：检查实际连接状态
    let connected_count = service.get_connected_count().await;
    log::info!("实际已连接数量: {}", connected_count);
    
    // 检查指定连接是否真的存在
    let connected_connections = service.get_connected_connections().await;
    let is_connected = connected_connections.iter().any(|conn| conn.id == connection_id);
    log::info!("连接 {} 是否在已连接列表中: {}", connection_id, is_connected);
...
/// 获取存储信息
#[tauri::command]
pub async fn get_storage_info(connection_id: String) -> Result<Vec<crate::models::StorageInfo>, String> {
    log::info!("前端请求获取存储信息: {}", connection_id);
    
    let service = SSH_SERVICE.read().await;
    service
        .get_storage_info(&connection_id)
        .await
        .map_err(|e| {
...
pub async fn get_network_info(connection_id: String) -> Result<crate::models::NetworkInfo, String> {
    ...
    let service = SSH_SERVICE.read().await;
    service
        .get_network_info(&connection_id)
        .await
        .map_err(|e| {
